...
Permettere di bloccare alcune dimensioni del pattern mentre si rigenera il resto.

- Lock consigliati: `pitch`, `rhythm`, `velocity`, `gate`.
- Esempio: cambiando `p`, `q`, `scene` o `source`, il ritmo puo' restare fisso mentre cambiano le note.
- Implementazione possibile: salvare gli eventi correnti e sovrascrivere i campi bloccati dopo la nuova generazione.
- I lock devono essere espliciti in UI e non devono modificare la sorgente deterministica di partenza.
...
Il menu scala della modalita' melodica puo' usare le scale interne del device oppure `live`. L'opzione `live` prova a seguire gli intervalli della scala globale di Ableton Live tramite `live.path live_set` e `live.observer scale_intervals`, lo stesso meccanismo di base usato da device come Sting 2. Viene ereditato il pattern intervallare; la root note resta un controllo locale di CryptoSeq.

La UI Max for Live usa un controllo lunghezza `1..128`, invia `rsa p q e` in modo atomico quando cambiano i primi, invia un `setup` silenzioso e debounced dopo raffiche di modifiche, e deriva la durata MIDI da tempo Live, divisione scelta, durata dell'evento e gate.
...
I pattern lock sono implementati nel model Max:
...
Quando un lock e' attivo, il model conserva l'ultimo pattern generato e sovrascrive i campi bloccati dopo una nuova generazione. Per esempio puoi cambiare source, RSA o scene mantenendo fisso il ritmo.
...
Con `morphamount 100` e `morphmode all`, il pattern risultante coincide con la scena B. Con valori intermedi, una maschera deterministica derivata dai valori generati decide quali step o quali campi provengono dal pattern B.

La UI del patcher 0.1 e' divisa in quattro aree di presentation:
