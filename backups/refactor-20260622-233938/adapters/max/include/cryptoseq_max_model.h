...
typedef enum cs_max_pattern_lock_t {
    CS_MAX_LOCK_PITCH = 1u << 0u,
    CS_MAX_LOCK_RHYTHM = 1u << 1u,
    CS_MAX_LOCK_VELOCITY = 1u << 2u,
    CS_MAX_LOCK_GATE = 1u << 3u
} cs_max_pattern_lock_t;

typedef enum cs_max_morph_mode_t {
    CS_MAX_MORPH_ALL = 0,
    CS_MAX_MORPH_PITCH = 1,
    CS_MAX_MORPH_RHYTHM = 2,
    CS_MAX_MORPH_VELOCITY = 3
} cs_max_morph_mode_t;

typedef struct cs_max_model_t {
    cs_params_t params;
    uint8_t pattern_locks;
    uint8_t morph_amount;
    uint8_t morph_scene;
    cs_max_morph_mode_t morph_mode;
