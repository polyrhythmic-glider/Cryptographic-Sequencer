...
static void cryptoseq_lock_target(cryptoseq_object_t *x, const char *target, long enabled)
{
    cryptoseq_post_status(
        x,
        cs_max_model_set_pattern_lock(&x->model, target, enabled != 0 ? 1u : 0u)
    );
}

static void cryptoseq_lockpitch(cryptoseq_object_t *x, long enabled)
{
    cryptoseq_lock_target(x, "pitch", enabled);
}

static void cryptoseq_lockrhythm(cryptoseq_object_t *x, long enabled)
{
    cryptoseq_lock_target(x, "rhythm", enabled);
}

static void cryptoseq_lockvelocity(cryptoseq_object_t *x, long enabled)
{
    cryptoseq_lock_target(x, "velocity", enabled);
}

static void cryptoseq_lockgate(cryptoseq_object_t *x, long enabled)
{
    cryptoseq_lock_target(x, "gate", enabled);
}
